...
class FirstScreen:
    ...
    def action(self):
        ...
        if diff == 4:
            ...
            vcmd = (self.window_custom.register(self.validate), "%d", "%i", "%P", "%s", "%S", "%v", "%V", "%W")
    ...
    def nextWind(self):
        ...
        if x not in range(4, 31) or y not in range(4, 31) or mp not in range(1, 100):
            messagebox.showerror(
                "Error",
                "The width and length of the board should me minimum 4 and maximum 30.\nThe percentage of mines goes from 1 to 99.",
            )
        else:
            self.window.withdraw()
...
class GameScreen:
    ...
    def getImages(self):
        ...
        self.images = {"plain": ImageTk.PhotoImage(image_plain), "flag": ImageTk.PhotoImage(image_flag), "numbers": []}
    ...
    def show_scores(self, win):
        ...
        for i, score in enumerate(user_best):
            ...
            tk.Label(frame_user, text=f"{np.round(score[1]/top_score*100,1)}%", width=7, anchor="e").grid(
        ...
        for i, score in enumerate(all_best):
            ...
            tk.Label(frame_all, text=f"{np.round(score[1]/top_score*100,1)}%", width=7, anchor="e").grid(
                row=i + 1, column=1, padx=1
            )
